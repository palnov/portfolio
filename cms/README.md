...
В `Projects → URL сайта проекта` указывайте HTTPS-адрес поддомена, например `https://aura.example.ru`. Он попадёт в предпросмотр внутри iframe и в кнопку «открыть отдельно». На период миграции валидны и старые локальные пути; перед публичным запуском замените их на реальные поддомены.
